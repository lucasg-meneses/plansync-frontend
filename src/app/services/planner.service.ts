...
export class PlannerService {
  ...
  updatePlanner(planner: Planner) {
    const jwtToken = localStorage.getItem(environment.tokenKey);
    const headers = new HttpHeaders().set('Authorization', `Bearer ${jwtToken}`);
    const url = this.apiUrlPlanner.concat('/' + planner.id);

    return this.http.put<Planner>(url, planner, { headers: headers })
      .pipe(catchError((error: HttpErrorResponse) => {
        if (error.status == 403) {
          this.authService.logout();
        }
  ...
  savePlanner(planner: Planner) {
    const jwtToken = localStorage.getItem(environment.tokenKey);
    const headers = new HttpHeaders().set('Authorization', `Bearer ${jwtToken}`);
    return this.http.post<Planner>(this.apiUrlPlanner, planner, { headers: headers })
      .pipe(catchError((error: HttpErrorResponse) => {
        if (error.status == 403) {
          this.authService.logout();
        }
